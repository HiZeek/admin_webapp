...
// import { Status } from "@wrapcbdc/db"
...
const MintPage = observer(() => {
  ...
  type MintRequest = {
    ...
    uuid: string
    date: Date
    status: "pending" | "failed" | "rejected" | "completed"
  }

  const approveMint = api.admin.transaction.approveMint.useMutation({
    ...
    onError() {
      return toast({
        title: "Error in approving mint",
        description: "Error while approving mint",
        variant: "destructive",
      })
    },
  })
  ...
  // Column definition
  const columns: ColumnDef<MintRequest>[] = [
    {
      accessorKey: "title",
      header: "Title",
      cell: ({ row }) => {
        ...
        return (
          <div
            className={cn("capitalize", {
              "text-red-400": status === "declined",
              "text-green-500": status === "minted",
        ...
        return request.status === "pending" ? (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              <DialogItem triggerChildren="Approve" triggerClass="mb-2">
                <div className="mb-4 grid items-center justify-items-center gap-4">
                  ...
                  <span className="text-center text-lg font-medium">
                    Confirm Minting approval
                  </span>
                  <span className="text-center text-sm">
                    Confirm approval of transaction with ID:
                    <b>{request.uuid}</b>
                  </span>
                </div>
                <DialogFooter>
              ...
              <DialogItem
                triggerChildren="Decline"
                triggerClass="border-red-400 text-red-500"
              >
                <div className="mb-4 grid items-center justify-items-center gap-4">
                  <svg
                    width="160"
                  ...
                  <span className="text-center text-lg font-medium">
                    Confirm Minting decline
                  </span>
                  <span className="text-center text-sm">
                    Confirm decline of transaction with ID:
                    <b>{request.uuid}</b>
                  </span>
                </div>
                <DialogFooter>
  ...
  const requests = useObservable<MintRequest[]>([])

  const getAllTransaction = api.admin.transaction.getAllTransaction.useQuery()


  useEffect(() => {

    if (getAllTransaction.isError) {
      toast({
        title: "Fetching transactions",
        description: getAllTransaction.error.message,
        variant: "destructive",
      })
    }

    if (getAllTransaction.data) {
      if (getAllTransaction.data.length) {
        requests.set(
          getAllTransaction.data.map((val, idx) => ({
            id: (idx + 1).toString(),
            title:
              val.trx_type === "crypto_deposit"
                ? "Crypto Deposit"
                : val.trx_type === "fiat_buy"
                  ? "Fiat Deposit"
                  : val.trx_type === "enaira_buy"
                    ? "Eniara Deposit"
                    : val.trx_type === "withdraw"
                      ? "Withdraw"
          ...
          <div className="col-span-3 h-full rounded-lg bg-slate-800/40">
            <Card className="border-none bg-slate-800/50">
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle>Mint and manage all token request</CardTitle>
              ...
              <CardContent>
                <div className="w-full overflow-hidden bg-slate-900/50 p-4">
                  <DataTable columns={columns} data={requests.get()} />
                </div>
              </CardContent>
            </Card>
          </div>
